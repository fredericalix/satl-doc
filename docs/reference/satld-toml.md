...
Point `satld` elsewhere with `--config` (see [`satld`](cli/satld.md)).

!!! info "Coming"

    The prose for each key is still to be written.
    The headings below are not decoration: `tools/check_config_keys.py` compares them against `struct ConfigFile` in the SatL source on every `make check`, in both directions — a key the daemon accepts and this page omits fails the build, and so does a key documented here that the daemon would refuse.
