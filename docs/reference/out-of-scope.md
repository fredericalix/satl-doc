...
- **The host must be FreeBSD 15.1 on amd64.**
  Nothing else is built or tested, and the daemon leans on FreeBSD-specific interfaces throughout — jails, VNET, `rctl(8)`, `pf(4)`, `if_vxlan`, `devfs` rulesets, and ZFS.
